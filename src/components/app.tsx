            ...
            >
              {list.map(({ name, desc }) => (
                <Card
                  title={name}
                  key={name}
                  className="w-64 rounded hover:ring-1 hover:shadow-xl ring-gray-200 ring-opacity-20 transition-shadow cursor-pointer"
                >
                  <p>{desc}</p>
                </Card>
              ))}
      ...
      <Layout className="app w-full h-full">
        <Row className="bg-white border-slate-900/10">
          <Col span={24} className=" pl-5 ">
            <Menu onClick={onClick} items={items} mode="horizontal"></Menu>
          </Col>
        </Row>
